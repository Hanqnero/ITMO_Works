...
= Цели и задачи
Определить толщину концевой меры методом прямых измерений и
...
#eqnn[
$
  overline(x) = strfmt("{:.3}", #data-avg) "мм"
$]
...
#eqnn[$
  S_overline(x) = strfmt("{:.4}", #data-msa) "мм"
$]
...
#eqnn[$
  t = 2.776 quad
  epsilon = strfmt("{:.4}", #data-epsilon) "мм"

$]

== Учет систематической погрешности

Известна инструментальная погрешность
...
#eqnn[$Delta overline(x) = strfmt("{:.4}", #abs-error) approx
 strfmt("{:.2}", #abs-error) "мм"
$]

Относительная погрешность

#eqnn[$
  delta x = (Delta overline(x))/overline(x) dot 100% = strfmt("{:.0}", #{abs-error/data-avg*100} )%
$]

= Результат

#eqnn[$
  l = (overline(l) plus.minus Delta overline(l)) = #data-avg plus.minus strfmt("{:.2}", #abs-error) "мм"
$]

= Выводы

+ Оценка на промахи по критерию Граббса показала, что промахов нет.
+ Вклад систематической погрешности оказался больше, чем вклад случайной
  погрешности, поэтому точность измерения ограничена используемыми приборами.
+ Поскольку относительная погрешность $delta x = 6% < 10%$, обработка
  результатов выполнена с высокой точностью.