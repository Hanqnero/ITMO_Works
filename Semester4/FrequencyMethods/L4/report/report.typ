...
удаляется шум. Наилучший результат получили при $d = omega_0$. Можем 
сказать, что гипотеза, выдвинутая вначале про параметр $omega_0$ 
подтвердилась.
...
точки $t=0$ сигналы, полученные двумя способами, немного расходятся.
...
Чем больше взятый период $T$, тем плавнее
получается график и тем медленнее он реагирует на изменение входных
...
#let last_cell = int(read("../python/cells/last_cell.txt").trim())

#for X in range(1, last_cell) [
